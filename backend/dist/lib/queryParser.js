...
export function parseQueryParams(params) {
    const result = { where: {} };
    const { espaco_id, espacoId, status, limit, order, checkin, checkin_gte, checkin_lte, checkin_lt, checkin_gt, data_inicio, data_fim, cpf, passport, ativo, ...rest } = params;
    ...
    if (checkin_lt) {
        ...
        if (parsed)
            dateFilters.lt = parsed;
    }
    if (checkin_gt) {
        const parsed = parseDate(checkin_gt);
        if (parsed)
            dateFilters.gt = parsed;
    }
    ...
    if (cpf)
        result.where.cpf = cpf;
    if (passport)
        result.where.passport = passport;
    // Ativo (spaces)
    if (ativo !== undefined) {
...
export function buildPrismaWhere(params) {
